...
public class RxExamplesStream {

	static ExecutorService executor = Executors.newFixedThreadPool(111);
	...
	private static void subscribetoMergedStream(
		...
		mergedObservable.subscribe(new Subscriber<String>() {
			...
			public void onCompleted() {
				System.out
						.println("----------------------- done with the stream of data ---------------------------------- --- ----");
			}
	...
	private static Observable<String> getAnamolyData(String book) {
		return Observable
				.create((Observable.OnSubscribe<String>) subscriber -> {
					...
					Runnable r = () -> {
						for (int i = 1; i < 100; i++) {
							subscriber.onNext(anomalysNetworkCall(i));
						}

						subscriber.onCompleted();
					};
					System.out.println("queuing anomaly -----");
					executor.execute(r);
	...
	private static Observable<String> getAlarmsData(String string) {
		return Observable
				.create((Observable.OnSubscribe<String>) subscriber -> {
					...
					Runnable r = () -> {
						for (int i = 1; i < 100; i++) {
							subscriber.onNext(alarmsNetworkCall(i));
						}
						subscriber.onCompleted();
					};
					executor.execute(r);
	...
	private static Observable<String> getHealthData(String string) {
		return Observable
				.create((Observable.OnSubscribe<String>) subscriber -> {
					...
					Runnable r = () -> {
						for (int i = 1; i < 100; i++) {
							subscriber.onNext(healthNetworkCall(i));
						}
						subscriber.onCompleted();
					};
					executor.execute(r);
	...
	private static String anomalysNetworkCall(int i) {
		...
		return "ANOMALY_DATA-" + i;
	}

	private static String alarmsNetworkCall(int i) {
		...
		return "ALARMS_DATA-" + i;
	}

	private static String healthNetworkCall(int i) {
